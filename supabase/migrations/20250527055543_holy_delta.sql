/*
  # Fix duplicate penthouse properties
  
  1. Changes
    - Remove all existing penthouse properties at the specified address
    - Add back exactly one Skyline Vista Penthouse (PH B6 302)
    - Add back exactly one Deer Valley Vista Penthouse (PH B6 301)
  
  2. Notes
    - Ensures no duplicates exist
    - Maintains correct unit numbers for each property
*/

-- First, remove all existing penthouses at this address to start fresh
DELETE FROM properties 
WHERE location->>'address' LIKE '1134 West Helling Circle%';

-- Add Skyline Vista Penthouse (PH B6 302)
...
-- Add Deer Valley Vista Penthouse (PH B6 301)
INSERT INTO properties (
  name,
  location,
  price,
